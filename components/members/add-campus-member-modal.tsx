...
interface AddCampusMemberModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function AddCampusMemberModal({
  isOpen,
  onClose,
  onSave,
  campusId,
  currentMembers,
}: AddCampusMemberModalProps) {
  ...
  useEffect(() => {
    const fetchAvailableUsers = async () => {
      ...
      try {
        ...
        if (response.ok) {
          // Filtrar usuários que não são membros do campus e estão ativos
          // Também filtrar usuários que não têm campus (sem campusMembers)
          const filteredUsers = users.filter(
            (u: any) => 
              !currentMembers.includes(u.id) && 
              u.active &&
              (!u.campusMembers || u.campusMembers.length === 0) // Usuários sem campus
          );
    ...
    fetchAvailableUsers();
  }, [campusId, currentMembers, isOpen]);
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-[var(--font-color)]">
              Adicionar Membro ao Campus
            </DialogTitle>
            <DialogDescription className="text-[var(--font-color)] opacity-70">
              Selecione um usuário sem campus para adicionar a este campus.
            </DialogDescription>
          </DialogHeader>
                ...
                <SelectContent className="border-[var(--border-color)]">
                  {availableUsers.length > 0 ? (
                    ...
                    <div className="text-sm text-muted-foreground mt-2 px-2">
                      {loading ? "Carregando..." : "Nenhum usuário sem campus disponível"}
                    </div>
                  )}
                </SelectContent>
              </Select>
