...
export function exportToPdfStyled(
  ...
  import("jspdf").then(({ default: jsPDF }) => {
    import("jspdf-autotable").then((autoTableModule) => {
      ...
      doc.setFontSize(14);
      doc.text(
        `Relatório da Comissão nº ${comissao} do Campus ${campus}`,
        40,
        40
      );
      ...
      sortedLocais.forEach((local, index) => {
        ...
        autoTable(doc as import("jspdf").jsPDF, {
          startY: startY + 10,
          head: [["Nome", "Local", "Estado", "Status"]] as [
            string,
            string,
            string,
            string
          ][],
          body: grouped[local].map(
            (item: BemCopia): [string, string, string, string] => [
              item.NOME || "",
              item.LOCALIZACAO || "",
          ...
          headStyles: {
            ...
            textColor: 255,
          } as import("jspdf-autotable").Styles,
          styles: {
            font: "helvetica",
            fontSize: 9,
            cellPadding: 4,
      ...
      // 💾 Salvar
      doc.save(
        `${fileName}_${items.length}_itens_${
