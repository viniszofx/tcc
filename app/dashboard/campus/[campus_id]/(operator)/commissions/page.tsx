...
export default async function CommissionsPage({ params }: CommissionsPageProps) {
  ...
  const commissions = data.commissions.filter((commission) => commission.campus_id === campus_id)
  ...
  return (
    <Card className="w-full max-w-3xl bg-[var(--bg-simple)] shadow-lg transition-all duration-300 lg:max-w-5xl xl:max-w-6xl">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl font-bold text-[var(--font-color)] md:text-2xl lg:text-3xl">Comissões</CardTitle>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {commissions.map((commission) => (
            <Card key={commission.id}>
              ...
              <CardFooter>
                <Link href={`/dashboard/campus/${campus_id}/commissions/${commission.id}`} className="w-full">
                  <Button className="w-full text-[var(--font-color2)] bg-[var(--button-color)] transition-all hover:bg-[var(--hover-3-color)]">Ver Comissão</Button>
                </Link>
              </CardFooter>
            </Card>
          ))}
